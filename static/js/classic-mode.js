...
function predictMatchLocally(match) {
    ...
    // 计算模型差值
    const home模型差值 = (adjustedProbs.home * match.home_odds) - 1;
    const draw模型差值 = (adjustedProbs.draw * match.draw_odds) - 1;
    const away模型差值 = (adjustedProbs.away * match.away_odds) - 1;

    // 确定概率参考
    const bestBet = home模型差值 > draw模型差值 && home模型差值 > away模型差值 ? 'home' :
                   draw模型差值 > away模型差值 ? 'draw' : 'away';

    const best模型差值 = Math.max(home模型差值, draw模型差值, away模型差值);
    ...
    return {
        ...
        recommendation: getBetRecommendation(bestBet, best模型差值)
    };
}
...
// 获取概率参考
function getBetRecommendation(bestBet, best模型差值) {
    const betNames = {
        'home': '主胜',
        'draw': '平局',
        'away': '客胜'
    ...
    if (best模型差值 > 0.05) {
        return `强烈参考 ${betNames[bestBet]} (模型差值: ${(best模型差值 * 100).toFixed(1)}%)`;
    } else if (best模型差值 > 0) {
        return `参考 ${betNames[bestBet]} (模型差值: ${(best模型差值 * 100).toFixed(1)}%)`;
    } else {
        return `谨慎参考 ${betNames[bestBet]} (模型差值: ${(best模型差值 * 100).toFixed(1)}%)`;
    }
}
        ...
        <div class="prediction-card classic-prediction">
            ...
            <div class="prediction-details">
                ...
                <div class="detail-item recommendation">
                    <strong>概率参考:</strong> ${pred.recommendation}
                </div>
            </div>
    ...
    function generateCombinations(index, currentCombo) {
        if (index === allSelections.length) {
            const combo = {
                ...
                avg模型差值: currentCombo.reduce((acc, sel) => acc + sel.ev, 0) / currentCombo.length
            };
    ...
    if (parlayPredictions.best) {
        bestParlayContainer.innerHTML = `
            <div class="best-parlay-card">
                <h3><i class="fas fa-star"></i> 组合参考方案</h3>
                <div class="parlay-details">
                    <div class="parlay-info">
                        ...
                        <span class="parlay-ev ${parlayPredictions.best.expectedValue > 0 ? 'positive' : 'negative'}">
                            模型差值: ${(parlayPredictions.best.expectedValue * 100).toFixed(1)}%
                        </span>
                    </div>
